...
public class FlightSection extends Section{
   ...
   constructor
   --------------------------------------*/
   FlightSection(int rows, int columns, Price price, SeatClass s) throws InvalidParameterException{
      checkParams(rows);
   
      this.price = price;
      this.section = new Seat[rows][columns];
      this.seatClass = s;
      this.markSection();
   }
   ...
   protected void markSection(){
      for(int row = 0; row < this.section.length; row++){
         for(int col = 0; col < this.section[row].length; col++){
            section[row][col] = new Seat(row,(char)(col + 'A'), this.price);
         }
      }
   }
